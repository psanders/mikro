...
- **BREAKING** `reviewedById`/`reviewedAt`/`reviewNote` are replaced by `assignedReviewerId`, `decidedById`, `decidedAt`, `decisionNote`, `rejectionReason`, `recommendation`, `approvedAmount`, `approvedTermWeeks`, `sentToDecisionAt`, plus a stored `aiSummary`. Data migrates.
...
- The founder app opens to REVIEWER users (scoped shell). Every application view and action lives in one right side panel (detail, edit, evidence, disbursement, assign); the admin decision is inline on its card. The Tareas "Nueva tarea" modal moves to the same side panel, so the founder app has no popups.
...
- New Playwright end-to-end suite for the founder app, run against a real apiserver on a seeded SQLite database, plus a full-lifecycle apiserver integration test.
...
- `founder-application-flow`: application cards, role-scoped feed, "Cerradas" group, side panel views, inline admin decision, reviewer access to the founder app.
