...
- **RESOLVED — "Contract available after Aprobada".** Decision: option (b) — the system **generates the loan contract PDF** (from a template) once an application is APPROVED, downloadable from the detail; it is then signed offline and re-uploaded (the existing `SIGNED` upload). This is a larger addition (contract template + generation + integration into the APPROVED state and the existing sign flow) and is **split into its own OpenSpec change** (`generate-loan-contract`), keeping this change focused on the request PDF, the contract download link, and the Pencil v2 alignment. The contract generator is **blocked on a contract template** (the legal text/fields), which must come from the business before it can be built.
