# copilot-feedback Specification

## Purpose

Lets the founder copilot file a GitHub issue mid-conversation — for a bug, a missing capability, or a UI/UX suggestion it notices — reusing the same issue-filing infrastructure as the human-facing feedback flow (`feedback-submission`). Exists so a gap the copilot notices (like a tool it can't call, or a confusing dashboard card) gets captured with real conversational context instead of depending on someone remembering to file it later.
