# reporting-foundation Specification

## Purpose

A single shared reporting module in `@mikro/common`: a report is defined once as a typed data model and rendered to both a JSON payload and a branded PDF, so every report entry point (CLI, dashboard, copilot) uses one module with no duplicated generation logic. Includes the branded multi-page PDF renderer (on the receipts font pipeline) and the shared FIFO waterfall-allocation + repayment-schedule helpers.
