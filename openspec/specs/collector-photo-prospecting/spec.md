# collector-photo-prospecting Specification

## Purpose

Enables registered COLLECTOR-role users to prospect new loan applicants via WhatsApp by photographing a prospect's phone number. The collector sends a photo of the number; the system extracts it with a vision LLM, asks for confirmation via interactive reply buttons, and — on approval — sends the intake Flow CTA promo template to that number.
