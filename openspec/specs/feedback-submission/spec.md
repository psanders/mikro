...
The reframed contract for the in-app feedback feature (formerly "bug report", mikro/#69): a
single generic channel for any user input — a bug, something confusing, or a feature idea —
submitted via `submitFeedback` and filed as a generic (unlabeled) GitHub issue the team
triages. Covers how the feature is presented (one flow, no category picker) and the desktop
entry point (square message icon with nav-rail tooltips). The platform-specific capture
mechanics live in `bug-report-desktop-capture` and `bug-report-mobile-capture`.
