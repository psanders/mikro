# mobile-evaluator-access Specification

## Purpose

Decoding roles from the mobile app's JWT and using them to gate navigation — which tab bar and screens a logged-in user sees, based on whether their roles include `REVIEWER`/`ADMIN` vs `COLLECTOR`-only, including a manual switcher for dual-role users.
