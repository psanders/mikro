...
The closed, code-defined catalog of automations a founder task may bind: the registry contract (slot sources, gate floors, deterministic execute) and the v1 automations pay-collector, record-expense, and daily-close (the ACCOUNTING.md collections-to-ledger bridge). Automations are shipped code — never user- or model-defined.
