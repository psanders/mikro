# loan-statement-report Specification

## Purpose

A per-loan statement report built on the reporting foundation: takes a loan id and produces JSON + a branded 2-page PDF (schedule + verification banner + received-payments ledger with reconciliation note), derived from the eval snapshot / schedule / health check — never hand-derived. Exposed identically via a founder-feed automation-catalog action and a CLI command.
