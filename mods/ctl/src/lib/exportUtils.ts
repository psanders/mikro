...
// ---------------------------------------------------------------------------
// Collections audit report (daily notifications audit)
// ---------------------------------------------------------------------------

/**
 * Output collections audit rows as a formatted table to a log function.
 */
export function outputCollectionsAuditAsTable(
...
/**
 * Write collections audit rows to a CSV file.
 */
export async function writeCollectionsAuditToCsv(
  rows: CollectionsAuditRow[],
  filepath: string
): Promise<{ rowCount: number }> {
  const header =
    "sentAt,customerName,customerPhone,loanId,loanNickname,attemptType,channel,status,templateName,messageId,notesOrError";
  const lines = rows.map((r) =>
    [
      ...
      r.customerPhone,
      r.loanId,
      `"${(r.loanNickname ?? "").replace(/"/g, '""')}"`,
      r.attemptType,
      r.channel,
      r.status,
      `"${(r.templateName ?? "").replace(/"/g, '""')}"`,
      `"${(r.messageId ?? "").replace(/"/g, '""')}"`,
      `"${(r.notesOrError ?? "").replace(/"/g, '""')}"`
    ].join(",")
  );
...
/**
 * Write collections audit rows to an Excel file.
 */
export async function writeCollectionsAuditToExcel(
  rows: CollectionsAuditRow[],
  filepath: string
): Promise<{ rowCount: number }> {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Auditoría de Cobranza");

  worksheet.columns = [
    { header: "Fecha/Hora", key: "sentAt", width: 22 },
    ...
    { header: "Plantilla", key: "templateName", width: 22 },
    { header: "Message ID", key: "messageId", width: 28 },
    { header: "Error / Notas", key: "notesOrError", width: 36 }
  ];

  worksheet.columns.forEach((c) => {
    if (c) c.alignment = { horizontal: "left", vertical: "top" };
  });
  const notesCol = worksheet.getColumn("notesOrError");
  if (notesCol) notesCol.alignment = { horizontal: "left", vertical: "top", wrapText: true };
  ...
  for (const r of rows) {
    const row = worksheet.addRow({
      ...
      loanId: r.loanId,
      loanNickname: r.loanNickname,
      attemptType: r.attemptType,
      channel: r.channel,
      status: r.status,
      templateName: r.templateName,
      messageId: r.messageId,
      notesOrError: r.notesOrError
    });
    row.eachCell((cell) => {
      cell.border = {
        top: borderStyle,
...
/**
 * Handle --output for collections audit: write to .csv or .xlsx when set; return true.
 * When output is not set, return false so the command can print the table to stdout.
 */
export async function handleCollectionsAuditOutput(
  rows: CollectionsAuditRow[],
  output: string | undefined,
  log: (msg: string) => void,
  error: (msg: string) => never
  ...
  const ext = output.split(".").pop()?.toLowerCase();
  if (ext === "csv") {
    const { rowCount } = await writeCollectionsAuditToCsv(rows, output);
    log(`CSV guardado: ${output} (${rowCount} intentos)`);
    return true;
  }
  if (ext === "xlsx") {
    const { rowCount } = await writeCollectionsAuditToExcel(rows, output);
    log(`Excel guardado: ${output} (${rowCount} intentos)`);
    return true;
  }
  error("--output must end in .csv or .xlsx");
}
