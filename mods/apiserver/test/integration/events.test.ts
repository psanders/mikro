...
describe("Founder Feed Integration", () => {
  ...
  describe("listApplicationEvents", () => {
    ...
    it("returns only events for the requested application, newest first", async () => {
      const app1 = await makeApplication();
      const app2 = await makeApplication({ sessionId: `sess-other-${Date.now()}` });
      await caller.approveApplication({ id: app1.id, note: "primero" });
      await caller.rejectApplication({ id: app2.id, reason: "otro" });

      const result = await caller.listApplicationEvents({ applicationId: app1.id });

      expect(result.items.map((e) => e.type)).to.deep.equal(["application.approved"]);
      expect(result.items[0].summary).to.contain("aprobada");
    });

    it("is accessible to REVIEWER, not just ADMIN", async () => {
      const app = await makeApplication();
      await caller.approveApplication({ id: app.id, note: "ok" });

      const result = await reviewerCaller().listApplicationEvents({ applicationId: app.id });
      expect(result.items).to.have.lengthOf(1);
    });

    it("rejects COLLECTOR-only callers", async () => {
      const app = await makeApplication();
      await expectForbidden(nonAdminCaller().listApplicationEvents({ applicationId: app.id }));
    });

    it("never returns payment events, even for the loan a converted application became", async () => {
      const { collector, loan } = await makeCustomerWithLoan();
      const app = await makeApplication();
      await caller.approveApplication({ id: app.id, note: "ok" });
      // A payment on an unrelated loan must not leak into this application's feed.
      await caller.createPayment({ loanId: loan.loanId, amount: 650, collectedById: collector.id });

      const result = await caller.listApplicationEvents({ applicationId: app.id });

      expect(result.items.map((e) => e.type)).to.not.include.members([
        "payment.collected",
        "payment.reversed"
      ]);
    });
